...
void TranspositionTable::zobrist_initialization()
{
	...
	for (int square = 0; square < 64; ++square) enpassant_keys[square] = rand();

	side_key = rand();
}

void TranspositionTable::hash_position(const BoardRepresentation& board_representation)
{
	...
	if (board_representation.get_enpassant() != 13) zobrist_key ^= enpassant_keys[board_representation.get_enpassant()];

	std::cout << zobrist_key << std::endl;
}
